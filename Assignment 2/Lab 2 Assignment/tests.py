...
class TestTripleDictGraph(unittest.TestCase):
    ...
    def test_bfs(self):
        graph = read_graph("test_in_graph.txt")
        visited, prev, dist = graph.bfs(0, 4)
        self.assertEqual(list(visited.values()), [True, True, True, True, False])
        self.assertEqual(list(prev.values()), [None, 0, 1, 1, None])
        self.assertEqual(list(dist.values())[:4], [0, 1, 2, 2])
        # Test that the distance to an inaccessible node is large
        self.assertGreater(list(dist.values())[4], 10000000)
        self.assertRaises(GraphException, graph.bfs, 9, 4)

    def test_lowest_length_path(self):
        graph = read_graph("test_in_graph.txt")
        path = graph.lowest_length_path(0, 3)
        _, _, dist = graph.bfs(0, 4)
        # Test that the path length is equal to the distance to the ending vertex going from the starting vertex
        self.assertEqual(len(path) - 1, dist[3])
        self.assertEqual(path, [0, 1, 3])
        self.assertRaises(GraphException, graph.lowest_length_path, 0, 9)
        self.assertRaises(GraphException, graph.lowest_length_path, 9, 0)
        self.assertRaises(GraphException, graph.lowest_length_path, 0, 4)
        self.assertRaises(GraphException, graph.lowest_length_path, 4, 0)

    def test_find_all_scc(self):
        graph = TripleDictGraph(5)
        # The strongly connected components of the following graph are: [0, 1, 2], [3], [4]
        graph.add_edge(0, 2, 1)
        graph.add_edge(0, 3, 1)
        graph.add_edge(1, 0, 1)
        graph.add_edge(2, 1, 1)
        graph.add_edge(3, 4, 1)
        scc = graph.find_all_scc()
        self.assertTrue([0, 1, 2] in scc or [0, 2, 1] in scc or [1, 0, 2] in scc or
                        [1, 2, 0] in scc or [2, 0, 1] in scc or [2, 1, 0] in scc)
        self.assertIn([3], scc)
        self.assertIn([4], scc)
        ...
        scc = graph.find_all_scc()
        self.assertIn([0], scc)
        self.assertTrue([1, 2] in scc or [2, 1] in scc)
        self.assertIn([3], scc)
        self.assertIn([4], scc)
    ...
    def test_kosaraju(self):
        ...
        scc = graph.kosaraju()
        self.assertIn([0, 4], scc)
        self.assertIn([1, 2], scc)
        self.assertIn([3], scc)

        graph2 = TripleDictGraph(5)
        graph2.add_edge(0, 2, 1)
        graph2.add_edge(0, 3, 1)
        graph2.add_edge(1, 0, 1)
        graph2.add_edge(2, 1, 1)
        graph2.add_edge(3, 4, 1)
        scc2 = graph2.kosaraju()
        self.assertIn([0, 1, 2], scc2)
        self.assertIn([3], scc2)
        self.assertIn([4], scc2)
