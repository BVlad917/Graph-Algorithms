...
class UI:
    def __init__(self, graph):
        ...
        self.__commands = {
            ...
            '18': self.__ui_draw_graph,
            '19': self.__ui_backwards_dijkstra,
            '20': self.__ui_count_distinct_minimum_walks,
        }
    ...
    def __print_menu():
        print("Graph operations:\n"
              ...
              "\t*18 - Draw the graph (Only use for small graphs!!!)\n"
              "\t*19 - Dijkstra between 2 vertices (Using the reverse Dijkstra implementation)\n"
              "\t*20 - Find the count of distinct walks of minimum cost between two vertices\n"
              "\tx - Exit\n"
              "* - Added in Assignment 3")

    def __ui_backwards_dijkstra(self):
        start = int(input("Give the starting vertex: "))
        end = int(input("Give the ending vertex: "))
        walk = self.__graph.reverse_dijkstra(start, end)
        walk_cost = self.__graph.calculate_walk_cost(walk)
        print(f"The shortest walk from vertex {end} to vertex {start} is:")
        print(' -> '.join([str(w) for w in walk]))
        print(f"With a cost of {walk_cost}.")

    def __ui_count_distinct_minimum_walks(self):
        start = int(input("Give the starting vertex: "))
        end = int(input("Give the ending vertex: "))
        count_of_walks = self.__graph.count_walks_of_minimum_cost(start, end)
        print(f"The number of minimum cost walks from vertex {start} to vertex {end} is {count_of_walks}.")

    def __ui_draw_graph(self):
        graph_drawing = self.__graph.get_graph_drawing()
        graph_drawing.render(view=True)
